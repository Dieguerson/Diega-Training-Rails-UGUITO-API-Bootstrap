  ...
  describe '#execute' do
    ...
    context 'with utility service' do
      ...
      let(:expected_notes_keys) do
        if utility.is_a?(NorthUtility)
          %i[id title note_type content created_at author book]
        else
          %i[id title review content created_at note_author_full_name author_email book_title book_author_name book_genre]
        end
    end
